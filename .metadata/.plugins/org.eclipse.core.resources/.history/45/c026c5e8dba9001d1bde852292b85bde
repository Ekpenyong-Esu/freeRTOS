...
int main(void)
{
  ...
  TimerHandle_t repeatHandle =                       // You can also create this handle in before main and initialize it to NULL before using it
  ...
  configASSERT(repeatHandle != NULL);      // test if the timer is succesfully created
  //start the scheduler - shouldn't return unless there's a problem

  xTimerStart(repeatHandle, 0);				// start the handle
  ...
  xTimerStart(oneShotHandle, 0);        // start the timer


  //wait for the push but
  while(!ReadPushButton());             // wait for the push button
...
/**
 * This is for one shot timer after executing once, the timer will go to hibernating mode
 */
...
/**
 * This is for repeated software timer
 */
void repeatCallBack( TimerHandle_t xTimer )
{
